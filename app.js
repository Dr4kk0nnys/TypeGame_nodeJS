...
console.log(`Start time:  ${GetDate()} \n\n`)
...
input.question('> ', (typed) => {
    ...
    */
    ...
    for (let i = 0; i < phrase.split(' ').length; i++) {
        if (phrase.split(' ')[i] == typed.split(' ')[i]) {
            correct++
        } else {
            wrong.value++
            wrong.words.push(`Correct: ${phrase.split(' ')[i]}  Typed: ${typed.split(' ')[i]}`)
        }
    }
    ...
    console.log('\n') // Aesthetic
    wrong.words.forEach((word) => {
        console.log(word)
    })
    console.log('\n') // Aesthetic


    let percentage = correct * 100 / phrase.split(' ').length
